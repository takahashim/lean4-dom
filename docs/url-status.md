...
| §3.2 / §3.5 host の往復 | `hostParser_ipv4`, `hostParser_opaque_id`, `hostParser_domain_id` | `Url/HostRoundtrip.lean` |
| §3.3 / §3.5 IPv6 の往復 | `ipv6Parser_serializer` | `Url/Ipv6Roundtrip.lean` |
...
| 定理 | 言っていること |
| --- | --- |
| `findRange_contains` | 返す区間は必ず `n` を含む。表が整っていなくても成り立つ |
| `findRange_complete` | **表が昇順・非重複なら、`n` を含む区間は必ず見つかる** |
| `findRange_mem` | 返す区間は配列の要素である |

`checkSorted` が昇順・非重複を読み込み時に確かめる。これが無いと、表が崩れたときに
「含む区間があるのに見つからない」が静かに起きる（`disallowed` かつ対象外と読むので
失敗側には倒れるが、気づけない）。
...
| 足場 | 言っていること |
| --- | --- |
| `hostParser_hostKind` | **host parser が返す host は、その scheme の表に合う**（special なら domain / IPv4、でなければ opaque / empty、IPv6 はどちらでも） |
| `PInv.hostNull`, `PInv.fileScheme`, `PInv.baseSchemeEq` | scheme を書き換える state では host が null、file slash / file host state では scheme が `file`、relative slash state では url の scheme が base の scheme |
| `hostKindOkOf_congr` | scheme を取り替えても表は通る。表が見るのは `isSpecialScheme` だけで、例外は empty host と `file` の一点だけ（protocol setter の四つ目の門がそこを止めている） |
...
well-founded 再帰なので簡約せず、URL の例には書けない。IPv6 だけの往復としては
`Url/Ipv6Roundtrip.lean` に定理の帰結として置いてある）。
...
帳尻は、parser の `compress` が圧縮位置 `c` ではなく `c + 1` になることで合う
（`take (c + 1)` に 0 が一つ含まれ、`replicate` が残り `L - 1` 個を戻す）。
