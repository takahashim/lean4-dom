...
| MutationObserver の配送 | record を積むところまで。microtask を回さない |
| attribute | model に attribute が無い |
| Shadow tree | 対象外 |
...
| `DOMException` に WebIDL の TypeError が無い | `moveBefore` の receiver が ParentNode でない場合を HierarchyRequestError で近似する |
